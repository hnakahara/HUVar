...
# GATM: single-aa -> Supporting (max 1). SLC9A6: <3 aa -> Supporting (max 2).
...
class TestSizeDowngrade:
    ...
    def test_two_aa_within_cutoff_supporting(self, tmp_path):
        # SLC9A6 cutoff is 2 aa; a 2-aa indel downgrades to Supporting.
        r = PM4Evaluator(_cfg(tmp_path)).evaluate(_DEL_2AA, _ann("SLC9A6"))
        assert r.triggered and r.strength == CriterionStrength.SUPPORTING
